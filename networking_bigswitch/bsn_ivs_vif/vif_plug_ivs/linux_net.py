...
from networking_bigswitch.bsn_ivs_vif.vif_plug_ivs import privsep
...
def create_ivs_vif_port(dev, iface_id, mac, instance_id, mtu=None):
    cmd = ['add-port', dev]
    _ivs_ctl(cmd)
...
@privsep.vif_plug.entrypoint
def update_ivs_vif_port(dev, mtu=None):
    _set_device_mtu(dev, mtu)
...
def _set_device_mtu(dev, mtu):
    """Set the device MTU."""
    # TODO(wolverineav): Check if _update_device_mtu() method from OVS
    # is of any use here. Instead of calling _set_device_mtu directly.
    if not mtu:
        return
