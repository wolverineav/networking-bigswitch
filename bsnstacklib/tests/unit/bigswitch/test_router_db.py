...
class RouterDBTestCase(RouterDBTestBase,
    ...
    def test_router_rules_update(self):
        with self.router() as r:
            ...
            min_prio = 3000
            r_id = r['router']['id']
            router_rules = [default_router_rule,
                            {'priority': -1,
        ...
        with self.router() as r1:
            with self.router() as r2:
                ...
                default_router_rule = {'priority': 3000,
                                       ...
                                       'action': 'permit',
                                       'nexthops': []}
                router1_rules = [default_router_rule,
                                 {'priority': 2990,
                                  'destination': '5.6.7.8/32',
                                  'source': '8.7.6.5/32',
                                  'action': 'permit',
                                  'nexthops': ['8.8.8.8', '9.9.9.9']}]
                router2_rules = [default_router_rule,
                                 {'priority': 2990,
                                  'destination': '1.2.3.4/32',
                                  'source': '4.3.2.1/32',
                                  'action': 'permit',
    ...
    def test_router_rules_validation(self):
        with self.router() as r:
            default_router_rule = {'priority': 3000,
                                   ...
                                   'nexthops': []}
            r_id = r['router']['id']
            good_rules = [default_router_rule,
                          {'priority': 2990,
                           'destination': '1.2.3.4/32',
                           'source': '4.3.2.1/32',
                           'action': 'permit',
